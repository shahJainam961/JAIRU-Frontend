...
export class ManagerDashboardComponent implements OnInit {
    ...
    this.managerService.getProjectDetails().subscribe(
      (response:any) => {
        ...
        response.forEach((element:any)=>{
          this.managerService.getFreeEmployeeForProject(element.projectId).subscribe(
            (response:any) => {
              console.log(response+"jainam");
              let freeEmployeeData:userDetails[] = [];
              response.forEach((freeEmployee:any) => {
                let userDetailsData = {
                  id:freeEmployee.userId,
          ...
          element.tasks.forEach((task:any) => {
            ...
            let taskDetailsData = {
              ...
              status:task.statusLu.taskStatusLuId,
              emp:userDetailsData,
            }
          ...
          let effortDataDetails:effortTable = {
            id:element.effortTable.effortTableId,
            effortSeries:effort,
          }
  ...
  addProjectDetails(){
    ...
    if(this.projectDetails.name=='' || this.projectDetails.description==''){
      this._snackBar.open("Please Enter all the required details", 'Close', {
  ...
  addEmployee()
  {
    console.log("addEmployee ma");
    if(this.addMember.value==null){
      console.log((this.addMember.value));
      this._snackBar.open("Please choose atleast one employee", 'Close', {
        horizontalPosition: this.horizontalPosition,
        verticalPosition: this.verticalPosition,
        duration:   3 * 1000,
      });
    ...
    this.managerService.addUserToProject(this.projects[this.projectDisplayIdx].id,userIds).subscribe(
      (response:any) =>{
        console.log(response);
        this.managerService.getFreeEmployeeForProject(response.projectId).subscribe(
          (response:any) => {
            // console.log(response);
            let freeEmployeeData:userDetails[] = [];
            response.forEach((freeEmployee:any) => {
  ...
  updateEffort(){
    this.managerService.updateEffortTable(this.projects[this.projectDisplayIdx].id,this.effortData[this.projectDisplayIdx]).subscribe(
      (response:any) => {
        ...
        let effortDataDetails:effortTable = {
          id:response.effortTable.effortTableId,
          effortSeries:effort,
        }
